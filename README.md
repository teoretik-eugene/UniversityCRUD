...
Для приложения используется реляционная БД PostgreSQL. Для того чтобы инициализировать данные, необходимо запустить sql запрос, который будет приведен в файле репозитория: ```init.sql```. Также необходимо указать в properties выбранную БД, username и password.
...
Если необходимо удалить университет, необходимо отправить запрос ```Delete http://localhost:8081/api/v1/university/{id}```, где ID - айди университета.
Запись будет удалена и вернется код 200.
